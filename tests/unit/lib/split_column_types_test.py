...
def test_split_column_types_no_categorical(many_dtypes_df: pd.DataFrame) -> None:
    test_continuous, test_categorical = split_columns_types(many_dtypes_df[["ints", "floats"]])
...
def test_split_column_types_different_int(
    many_dtypes_df: pd.DataFrame, dtype_int: type[np.int_]
) -> None:
    test_categorical = [2, 3]
    test_continuous = [0, 1]
...
def test_split_column_types_different_float(
    many_dtypes_df: pd.DataFrame, dtype_float: type[np.float64]
) -> None:
    test_categorical = [2, 3]
    test_continuous = [0, 1]
