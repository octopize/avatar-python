...
def test_postprocess_noop(many_dtypes_df: pd.DataFrame) -> None:
    ...
    processor = GroupModalitiesProcessor(min_unique=3, global_threshold=1, new_category="other")
...
def test_group_preprocess_new_category(categorical_df: pd.DataFrame) -> None:
    ...
    df = categorical_df
    processor = GroupModalitiesProcessor(min_unique=2, global_threshold=1, new_category="andere")
...
def test_group_preprocess_variable_thresholds(categorical_df: pd.DataFrame) -> None:
    ...
    df = categorical_df
    processor = GroupModalitiesProcessor(variable_thresholds={"variable_1": 1, "variable_2": 2})
...
def test_wrong_variable_thresholds_raises_error(categorical_df: pd.DataFrame) -> None:
    ...
    df = categorical_df
    with pytest.raises(ValueError, match="Expected valid variables in variable_thresholds*"):
...
def test_with_global_threshold_without_min_unique() -> None:
    ...
    with pytest.raises(ValueError, match="Expected both of (global_threshold, min_unique)*"):
