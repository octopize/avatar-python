...
- [User guide](#user-guide)
  ...
  - [How to view all jobs](#how-to-view-all-jobs)
  - [How to cancel a job](#how-to-cancel-a-job)
...
If you prefer to skip this compatibility check, you can do so :

```python
client = ApiClient(base_url=url, should_verify_compatibility=False)
```
...
## How to have advice on parameters and processors

You can have recommendations on which parameters and processors to use in the avatarization.
...
print(advice_job.result.more_details)  # additional recommendations

pipeline = advice_job.result.python_client_pipeline.replace("<NAME_OF_YOUR_DF>", "df")
result = client.pipelines.avatarization_pipeline_with_processors(eval(pipeline))
```
...
An avatarization job can be launched on data containing time series. For more details on time series avatarization, see the [dedicated notebook tutorial on time series](https://github.com/octopize/avatar-python/blob/main/notebooks/Tutorial7-Time_series.ipynb) and the [dedicated page in our public documentation](https://docs.octopize.io/docs/understanding/timeseries/).
...
```python
client.jobs.find_all_jobs_by_user()
```

## How to cancel a job

A job can be cancelled. This is useful to manage the queue of jobs created and avoid creation of unnecessary long queues.
