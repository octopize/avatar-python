...
def modify_class_name(soup : BeautifulSoup) -> BeautifulSoup:
    ...
    client_class_definitions = ([c.parent for c in all_classes if c.text == "avatars.api."])

    module_names = [c.find_all("span", {"class" : "pre"})[1] for c in client_class_definitions]
    class_names = [c.find_all("span", {"class" : "pre"})[2] for c in client_class_definitions]
