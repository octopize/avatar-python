...
class InterRecordCumulatedDifferenceProcessor:
    ...
    Examples:
        ...
        different than the original one. In such cases, the processor should be
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        if self.id_variable not in df.columns.values:
            raise ValueError(f"Expected a valid `id_variable`, got {self.id_variable} instead")
        ...
        df[self.new_difference_variable_name] = df[self.new_difference_variable_name].fillna(0)
    ...
    def postprocess(self, source: pd.DataFrame, dest: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.keep_record_order:
            ordered_indices = source.sort_values([self.id_variable, self.target_variable]).index
            df = df.loc[ordered_indices]

        # calculate target value as first value + cumulated sum of the differences
        df[self.target_variable] = df[self.new_first_variable_name] + df.groupby(self.id_variable)[
            self.new_difference_variable_name
        ].transform(pd.Series.cumsum)

        df = df.drop(columns=[self.new_first_variable_name, self.new_difference_variable_name])
