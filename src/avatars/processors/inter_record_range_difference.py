...
class InterRecordRangeDifferenceProcessor:
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[self.new_range_variable] = df[self.target_end_variable] - df[self.target_start_variable]
        ...
        df["__end_tmp__"] = df.groupby(self.id_variable)[self.target_end_variable].shift()

        # store the difference between current and previous value
        df[self.new_difference_variable] = df[self.target_start_variable] - df["__end_tmp__"]
    ...
    def postprocess(self, source: pd.DataFrame, dest: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.keep_record_order:
            ordered_indices = source.sort_values([self.id_variable, self.sort_by_variable]).index
            df = df.loc[ordered_indices]

        # calculate for each row the sum of ranges of all previous records
        df["__range_tmp__"] = df.groupby(self.id_variable)[self.new_range_variable].shift()
        df["__range_tmp__"] = df["__range_tmp__"].fillna(0)
        df["__totalizer_tmp__"] = df.groupby(self.id_variable)["__range_tmp__"].transform(
            pd.Series.cumsum
        ) + df.groupby(self.id_variable)[self.new_difference_variable].transform(pd.Series.cumsum)
        ...
        df[self.target_start_variable] = df[self.new_first_variable] + df["__totalizer_tmp__"]

        # calculate end values from start value and range value
        df[self.target_end_variable] = df[self.target_start_variable] + df[self.new_range_variable]
