...
class ToCategoricalProcessor:
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.keep_continuous:
            self.continuous_variables = [x + self.continuous_suffix for x in self.variables]
    ...
    def postprocess(self, source: pd.DataFrame, dest: pd.DataFrame) -> pd.DataFrame:
        ...
        dest[self.variables] = dest[self.variables].astype(source[self.variables].dtypes.to_dict())
