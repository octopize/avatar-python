...
# %% [markdown]
# In this tutorial, we will learn how to prepare data prior to avatarization using custom processors.
#
# Some transformations can run locally as client-side processors, while others can run server-side directly in the avatarization pipeline.
#
# We'll show how to combine both approaches to integrate domain knowledge into an avatarization workflow.
...
import numpy as np
import pandas as pd
from avatar_yaml.models.parameters import RelativeDifferenceParameters
...
#
# The `ProportionProcessor` is applied client-side. The relative-difference transformation is configured as a custom server-side processor and run automatically by the avatarization pipeline.
#
# This setup demonstrates how to combine local preprocessing and server-side custom processors in a single avatarization workflow.
#
#
...
# %% [markdown]
# Some variables may have a hierarchy where one variable is always higher than another. In order to preserve this hierarchy during avatarization, one variable can be expressed as the difference of another.
...
# In this tutorial, this transformation is configured as a server-side custom processor.

# %%
relative_difference_parameters = RelativeDifferenceParameters(
...
runner_with_processor.set_parameters(
    "game",
    k=5,
    use_categorical_reduction=True,
    processors=[relative_difference_parameters],
)
